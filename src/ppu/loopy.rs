use super::{Point, PixelCoord, VAddr, NTAddr, TileCoord};
...
impl AddrReg {
	...
	pub fn update(&mut self) {
		self.address = self.temp;
	}

	pub fn get_addr(&self) -> VAddr {
		VAddr::new_wrapping(self.address)
	}
	...
	pub fn set_fine_x(&mut self, val: PixelCoord) {
		self.fine_x = val;
	}
	...
	pub fn get_coarse_x(&self) -> TileCoord {
		TileCoord::new_wrapping(self.address as u8)
	}

	pub fn get_coarse_y(&self) -> TileCoord {
		TileCoord::new_wrapping((self.address >> 5) as u8)
	}

	pub fn get_nametable(&self) -> NTAddr {
		NTAddr::new_wrapping((self.address >> 10) as u8)
	}
	...
	pub fn get_fine_y(&self) -> PixelCoord {
		PixelCoord::new_wrapping((self.address >> 12) as u8)
	}

	pub fn get_fine_x(&self) -> PixelCoord {
		self.fine_x
	}
}